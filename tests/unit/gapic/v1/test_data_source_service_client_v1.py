...
class TestDataSourceServiceClient(object):
    def test_update_transfer_run(self):
        ...
        client = datatransfer_v1.DataSourceServiceClient(channel=channel)
        ...
        expected_request = datasource_pb2.UpdateTransferRunRequest(
            transfer_run=transfer_run, update_mask=update_mask)
    ...
    def test_log_transfer_run_messages(self):
        ...
        assert len(channel.requests) == 1
        expected_request = datasource_pb2.LogTransferRunMessagesRequest(
            name=name, transfer_messages=transfer_messages)
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request
    ...
    def test_start_big_query_jobs(self):
        ...
        name = client.run_path('[PROJECT]', '[LOCATION]', '[TRANSFER_CONFIG]',
                               '[RUN]')
        imported_data = []
        user_credentials = b'-120'

        client.start_big_query_jobs(name, imported_data, user_credentials)

        assert len(channel.requests) == 1
        expected_request = datasource_pb2.StartBigQueryJobsRequest(
            name=name,
    ...
    def test_create_data_source_definition(self):
        ...
        expected_response = {
            'name': name,
            'transfer_run_pubsub_topic': transfer_run_pubsub_topic,
            'support_email': support_email,
            'disabled': disabled,
            'transfer_config_pubsub_topic': transfer_config_pubsub_topic
        }
        expected_response = datasource_pb2.DataSourceDefinition(
            **expected_response)
        ...
        response = client.create_data_source_definition(
            parent, data_source_definition)
        assert expected_response == response

        assert len(channel.requests) == 1
        expected_request = datasource_pb2.CreateDataSourceDefinitionRequest(
            parent=parent, data_source_definition=data_source_definition)
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request
    ...
    def test_create_data_source_definition_exception(self):
        ...
        with pytest.raises(CustomException):
            client.create_data_source_definition(parent,
                                                 data_source_definition)
    ...
    def test_update_data_source_definition(self):
        ...
        response = client.update_data_source_definition(
            data_source_definition, update_mask)
        assert expected_response == response

        assert len(channel.requests) == 1
        expected_request = datasource_pb2.UpdateDataSourceDefinitionRequest(
    ...
    def test_update_data_source_definition_exception(self):
        ...
        with pytest.raises(CustomException):
            client.update_data_source_definition(data_source_definition,
                                                 update_mask)
    ...
    def test_delete_data_source_definition(self):
        ...
        # Setup Request
        name = client.data_source_definition_path('[PROJECT]', '[LOCATION]',
                                                  '[DATA_SOURCE_DEFINITION]')

        client.delete_data_source_definition(name)

        assert len(channel.requests) == 1
        expected_request = datasource_pb2.DeleteDataSourceDefinitionRequest(
            name=name)
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request
    ...
    def test_get_data_source_definition(self):
        ...
        assert len(channel.requests) == 1
        expected_request = datasource_pb2.GetDataSourceDefinitionRequest(
            name=name)
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request
    ...
    def test_list_data_source_definitions(self):
        ...
        expected_response = datasource_pb2.ListDataSourceDefinitionsResponse(
            **expected_response)
        ...
        assert len(channel.requests) == 1
        expected_request = datasource_pb2.ListDataSourceDefinitionsRequest(
            parent=parent)
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request
